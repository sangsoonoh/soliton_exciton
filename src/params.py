...
class Params():
	def __init__(self,
		...
		self.Lx = (Nx*3*b) - b + 2*padding
		self.Ly = a

		self.x_min = -0.5*self.Lx
		self.x_max = 0.5*self.Lx


		self.y_min = -0.5*self.Ly
		self.y_max = 0.5*self.Ly
		...
		self.g1 = 2*m.pi/(self.Lx) *g1_norm
		self.g2 = 2*m.pi/(self.Ly) *g2_norm

		self.G1 = 2*m.pi/(self.Lx)*np.array([1, 0]) 
		self.G2 = 2*m.pi/(self.Ly)*np.array([0, 1])
